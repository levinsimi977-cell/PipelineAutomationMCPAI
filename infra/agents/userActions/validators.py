...
def validate_discovery(discovered: dict[str, Any]) -> dict[str, Any]:
    ...
    documentation (layoutFile), and manifest documentation (source contains
    "manifest").
    """
    ...
    manifest_documented = all("manifest" in event.get("source", []) for event in events)

    passed = has_events and trigger_ids_present and xml_documented and manifest_documented
